...
const ManageCourses = () => {
      ...
      course.id === courseId
        ? {
            ...
            lessons: course.lessons
              ? course.lessons.filter((lesson) => lesson.id !== lessonId)
              : [],
          }
        : course
    );
  ...
  const editLesson = (courseId, lesson) => {
    navigate(`/admin/edit-lesson/${lesson.id}`, { state: { lesson, courseId } });
  };

  const addLesson = (course) => {
    navigate(`/admin/add-lesson/${course.id}`, { state: { course } });
  };
                      ...
                      <ul className="space-y-2">
                        {course.lessons.map((lesson) => (
                          ...
                          >
                            ...
                            <div className="flex items-center gap-3">
                              <button
                                onClick={() => editLesson(course.id, lesson)}
